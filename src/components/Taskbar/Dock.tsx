...
export function Dock() {
  ...
  return (
    <div className="fixed bottom-[52px] sm:bottom-[56px] left-1/2 -translate-x-1/2 z-[5000]">
      <motion.div
        className="flex items-end gap-0 px-2 sm:px-3 py-2 sm:py-2.5 glass rounded-[24px] sm:rounded-[28px] shadow-2xl"
      ...
      >
        {portfolioData.desktopIcons.map((item, i) => {
          ...
          const scale = isHovered ? 1.45 : Math.max(0.9, 1.1 - distance * 0.1);

          return (
            <div key={item.id} className="relative flex flex-col items-center gap-0.5 px-[3px] sm:px-0.5"
              onMouseEnter={() => setHoveredIndex(i)}
              onMouseLeave={() => setHoveredIndex(null)}
              onTouchStart={() => setHoveredIndex(i)}
              onTouchEnd={() => { setTimeout(() => setHoveredIndex(null), 1500); }}>
              <motion.button
                className="relative w-[36px] h-[36px] sm:w-[44px] sm:h-[44px] rounded-full flex items-center justify-center shrink-0 cursor-pointer"
                style={{
                ...
                whileTap={{ scale: 0.85 }}
              >
                <AppIcon id={item.id} size={20} className="size-[16px] sm:size-[20px]" />
              </motion.button>
